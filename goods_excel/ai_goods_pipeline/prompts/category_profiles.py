    ...
    *, category_id: int, target_count: int, city_strategy: str
    ...
    if category_id == 126:
        rules = [
            ...
            "126 多样性要求:",
            "- 优先覆盖江苏不同城市的特产，不要集中在南京、苏州两个城市。",
            "- 熟食卤味、茶饮酒水、糕点零食、酱菜调味、水产干货、节令水果、肉脯肉干等方向轮着出，不要反复生成同一种类型。",
            "- 同一个城市也不要每次出同样的商品（比如南京不只有盐水鸭，还有雨花茶、鸭血粉丝汤料包、龙袍蟹黄汤包、六合活珠子等）。",
            "- 禁止只是重复生成盐水鸭、镇江香醋、碧螺春、靖江猪肉脯这几种，这批出了下次换别的。",
            "- 拿不准时宁可采用保守表述，不要虚构老字号、地理标志或历史典故。",
        ]
        if target_count >= 8:
    ...
    if category_id == 127:
        rules = [
            ...
            "127 多样性要求:",
            "- 覆盖米面杂粮、时令蔬菜、鲜活水产、禽蛋禽肉、海产干货、糖果甜酒、滋补食材等方向，不要只反复生成大闸蟹、大米、咸鸭蛋。",
            "- 优先覆盖 6 个以上不同江苏城市，再考虑同城扩展第二个农副品。",
        ]
    ...
    if category_id == 128:
        rules = [
            ...
            "128 多样性要求:",
            "- 车贴、冰箱贴、徽章、帆布袋、钥匙扣、水杯、笔记本、摆台、公仔、挂件等载体轮着出，不要只反复生成同一种。",
            "- 尽量覆盖更多江苏城市，不要集中在南京、苏州两三个热点城市。",
        ]
        if city_strategy == "balanced" and target_count >= 8:
    ...
    if category_id == 129:
        rules = [
            ...
            "129 多样性要求:",
            "- 紫砂茶器、苏绣织物、竹木雕刻、漆器、金箔工艺品、陶瓷摆件、团扇折扇、书签等轮着出，不要只集中在紫砂壶或苏绣屏风。",
            "- 紫砂壶也不要只出西施壶、石瓢壶，可以试其他器型（井栏、合欢、龙蛋等）。",
        ]
        if target_count >= 8:
