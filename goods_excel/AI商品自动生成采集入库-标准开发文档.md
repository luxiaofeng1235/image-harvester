...
- 图片有效性校验不能只看 `200`、`Content-Type=image/*` 或 `Content-Length`，还必须对返回字节做一次真实图片解码；解码失败、损坏图、伪图片内容都视为无效图直接丢弃。
- OSS 上传阶段同样必须再次校验图片真实可解码，避免把防盗链返回页、权限拦截页或损坏内容直接同步到 OSS。
- 若图片 URL 在归一化后已经是前端可识别的标准图片格式，则主图可直接写入数据库，不强制再走 OSS。
...
- `OSS_ENABLED=1` 时，详情图应统一同步到 OSS 后再写入富文本，避免外链防盗链、Referer 校验或前端兼容性导致的详情图渲染失败；主图则保留“标准直链直接写库、非标准直链转 OSS”的策略。
...
### 13.1 功能验收
- 能按分类稳定生成指定数量商品。
- 每条商品固定 `1` 张主图 + `3` 张详情图；开启 OSS 时详情图统一为 OSS 域名，主图允许为标准直链或 OSS 域名。
...
- 映射层: 抓回图片后继续做可访问性、类型、大小、真实解码、去重和离题过滤，最终固定映射为 `1 张主图 + 3 张详情图`；写库前先做图片 URL 归一化，主图保持“标准图链直接写库、非标准图链转 OSS”，详情图在开启 OSS 时统一同步到业务访问域名。
